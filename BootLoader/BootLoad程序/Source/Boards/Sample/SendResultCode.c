...
STATIC VOID SendMotorPos(VOID)
{
	...
	sendBuf[0] = 0;  //状态单元
	sendBuf[1] = 0;
	sendBuf[2] = HBYTE (LWORD(xpluse));
	sendBuf[3] = LBYTE (HWORD(xpluse));
	sendBuf[4] = LBYTE (LWORD(xpluse));
	sendBuf[5] = HBYTE (LWORD(ypluse));
	sendBuf[6] = LBYTE (HWORD(ypluse));
	sendBuf[7] = LBYTE (LWORD(ypluse));
	sendBuf[8] = HBYTE (LWORD(zpluse));
	sendBuf[9] = LBYTE (HWORD(zpluse));
	sendBuf[10] = LBYTE (LWORD(zpluse));
	sendBuf[11] = HBYTE (LWORD(synpluse));
	sendBuf[12] = LBYTE (HWORD(synpluse));
