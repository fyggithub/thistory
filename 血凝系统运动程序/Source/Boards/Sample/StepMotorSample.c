...
VOID StepZAxisMotorMovRel (StepMotor* motor, INT32 step, UINT32 speed)
{
	if (step < 0){
		...
		MainState.motors[MOTOR_Z].speedPlan.accTable   = GetAccelerateTable (ACCTBL_SAMPLE_Z_UP);
	}
...
{
	if (step < 0){
		MainState.motors[MOTOR_SYN].speedPlan.accTable   = GetAccelerateTable (ACCTBL_SAMPLE_Z_DOWN);
	}
	else{
		MainState.motors[MOTOR_SYN].speedPlan.accTable   = GetAccelerateTable (ACCTBL_SAMPLE_SYN_UP);
	}
	StepMotorMovRel(motor, step, speed);
}
/********************************************************************
