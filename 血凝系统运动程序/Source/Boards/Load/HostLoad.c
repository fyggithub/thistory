 ...
 *延时指令
 *******************************************************************/
STATIC BOOL HostCmdDelay(HostFrame* frame)
{
	Message msg;
	if ( frame && frame->dataLen >= 2){
		UINT16 delay = frame->data[0]*256 + frame->data[1];
		msg.id = MSG_HOST_DELAY;
		msg.p1 = delay;
		msg.p2 = msg.p3 = msg.p4 = 0;
		MessagePost(&msg);
		return TRUE;
 ...
 *停机指令
 *******************************************************************/
STATIC BOOL HostCmdStop(HostFrame* frame)
{
	Message msg;
	if ( frame && frame->dataLen >= 1){
		UINT16 i = frame->data[0];
		msg.id = MSG_HOST_STOP;		
		msg.p1 = i;
		msg.p2 = msg.p3 = msg.p4 = 0;
		MessagePost(&msg);
		return TRUE;
...
STATIC UINT32 HostLoadParser(HostFrame* frame)
{
	...
	switch(frame->command){
		...
		case HOST_CMD_STOP			: ret = HostCmdStop(frame);			break;
		case HOST_CMD_DELAY			: ret = HostCmdDelay(frame);		break;
		default:
