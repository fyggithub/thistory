...
STATIC VOID BoardGpioInit(VOID)
{
	...
	GpioInit(&(board.gpios[IO_LIQUIDE_OUTPUT]),
		GPIO_MODE_OUTPUT_PP, GPIO_SPEED_FREQ_LOW);
	
	board.gpios[IO_LIQUID_INPUT].io  = GPIOG;
	board.gpios[IO_LIQUID_INPUT].pin = GPIO_PIN_3;
	GpioExitInit(&(board.gpios[IO_LIQUID_INPUT]), 
				GPIO_MODE_IT_FALLING, GPIO_PULLDOWN,EXTI9_5_IRQn);

//Mix_motor text
	board.gpios[IO_MIX_DRECTION].io  = GPIOD;
	board.gpios[IO_MIX_DRECTION].pin = GPIO_PIN_12;
	GpioInit(&(board.gpios[IO_MIX_DRECTION]),
		GPIO_MODE_OUTPUT_PP, GPIO_SPEED_FREQ_LOW);
}
