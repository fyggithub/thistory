	...
	switch(MainState.stateManageBtm)
	{
		...
		case STATE2_CLN_CLN_WAIT:{
						if(msg->id == MSG_TIMER){			//目前只支持外壁时间大于内壁时间			
							...
							StepZAxisMotorMovAbs(&(MainState.motors[MOTOR_Z]), 0, REAGENT_ZAXISDRAINCLE_RISESPEED);  //z轴电机回原点
...
VOID RegeStateClnMessageHandler(Message* msg)
{
	switch(MainState.stateManageMid)
	{
		...
		case STATE1_CLN_DOWN:{																
							StepZAxisMotorMovAbs (&(MainState.motors[MOTOR_Z]),ConfigGet(CFG_REAGENT_CLEAN_UP_POS), REAGENT_ZAXISDRAINCLE_FALLSPEED); //Z轴下降到清洗位									
